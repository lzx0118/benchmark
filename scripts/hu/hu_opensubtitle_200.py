...
"""
从 Europarl.en-hu.merged.tsv 中抽取高质量匈牙利语句子（hu 列），
尽量保证句子内部不含其它语言的长单词。
...
过滤条件：
  - 只读取前 MAX_LINES 行（不含表头）
  - hu 词数 >= MIN_TOKENS
  - 去重：同一 hu 句子只保留一次
  - 匈牙利重音字母比例 >= MIN_HU_SPEC_RATIO
  - 不允许出现“疑似外语长单词”（长度 >= 4 且不含任何匈牙利重音字母）

输出：id, hu 两列（你如果只想要 hu，可以在后面改成只写 hu）
"""
...
# ========= 路径 & 参数 =========
BASE_DIR   = r"<PATH_TO_XBENCH_ROOT>/hu\Opensubtitle"
INPUT_TSV  = os.path.join(BASE_DIR, "OpenSubtitles.en-hu.merged.tsv")
OUTPUT_TSV = os.path.join(BASE_DIR, "OpenSubtitles.en-hu.sample200.tsv")

MAX_LINES         = 150000   # 只看前 1000 条（不含表头）
